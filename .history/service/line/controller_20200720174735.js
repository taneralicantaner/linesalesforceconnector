...
const getLinkToken = (body) => {
	const request = require("request");
   const headers = {
	  Authorization: "Bearer {" + process.env.LINE_CHANNEL_ACCESS_TOKEN + "}"
	};
	const options = {
	  url: "https://api.line.me/v2/bot/user/"++"/linkToken",
	  proxy: process.env.FIXIE_URL,
	  headers: headers,
	};
	return new Promise((resolve, reject) => {
	  request.post(options, (error, response, body) => {
		if (error || response.statusCode != 200) {
		  ...
		  resolve(body);
		  b = {
			"to": "Ue2fb00cbcc122443885e2cbd0e41f9c7",
			"messages": [{
				"type": "template",
				...
				"template": {
					...
					"actions": [{
						"type": "uri",
						"label": "Account Link",
						"uri": "https://brave-bear-446336-dev-ed--c.ap4.visual.force.com/apex/tes?linkToken="+body.linkToken
					}]
				}
			}]
		}
