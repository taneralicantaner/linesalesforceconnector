...
const LINE_AUTO_RESPONSE = require("../../common/constants").LINE_AUTO_RESPONSE;
...
const createEvent = async (profile, event) => {
  ...
  if (event.type === "message") {
    ...
    e.message = {
      type: "text",
      text: LINE_AUTO_RESPONSE.ACCOUNTLINKMESSAGE + linkToken,
    };
    ...
    if (event.link.result == "failed") {
      e.message = {
        type: "text",
        text: LINE_AUTO_RESPONSE.ACCOUNTLINKFAILEDMESSAGE,
      };
    } else {
      updateSalesforceUser(event);
      e.message = {
        type: "text",
        text: LINE_AUTO_RESPONSE.ACCOUNTLINKSUCCEDEDMESSAGE,
      };
    }
