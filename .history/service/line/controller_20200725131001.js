...
const createEvent = async (profile, event) => {
  ...
  if (event.type === "message") {
    ...
    linkToken = await getLinkToken(e).catch((err) => console.log(error));
    e.message = {
      type: "text",
      text:
        "友達登録ありがとうございます！\nLineで物件に関する問合せするおよび最新のお知ら情報せを受け取るため、Lineアカウントとコミュニティアカウントの連携が必要です。アカウントを連携するため、リンクをクリックしてコミュニティにログインしてください。\n" +
    ...
    await updateSalesforceUser(event);
    e.message = {
      type: "text",
      text:
        "アカウントの連携ありがとうございます。物件に関するお困りの点がございましたらLineで気軽に問合せしてください。",
    };
