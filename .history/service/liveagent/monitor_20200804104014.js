  ...
  request.get(options, async (error, response, body) => {
    if (response.statusCode === 204) {
      ...
      body.messages.forEach(function(message) {
        if (message.type == "ChatEstablished") console.log(message);
...
const onChatEnded = async (session) => {
  ...
  await Router.processEvent(
    ...
    createEvent(session, "message", { type: "text", text: "チャットを終了しました。" })
  );
  endMonitorChatActivity(session);
};
...
const onChatTransferred = async (session) => {
  info("LIVEAGENT:onChatTransferred:オペレータに転送");
  await Router.processEvent(createEvent(session, "message", { type: "text", text: "オペレータに転送されました。" }));
};
