...
const monitorChatActivity = async (sessionId) => {
  ...
  request.get(options, async (error, response, body) => {
    if (response.statusCode === 204) {
      ...
      //   session.ack = session.ack + 1;
      session = await updateSessionAck(session);
      body.messages.forEach(function(message) {
        console.log(message);
        processMessage(session, message, monitorChatActivity(session.id));
      });
      //   console.log(body.messages[0]);
      //   processMessage(session, body.messages[0]);
      //   monitorChatActivity(session.id);
    } else {
...
const endMonitorChatActivity = async (session) => {
  ...
  await Router.deleteRouteByTerminal(session.id);
};

const processMessage = async (session, message, callback) => {
  switch (message.type) {
    case "ChatMessage":
      await onChatMessage(session, message);
      break;
    case "RichMessage":
      onRichMessage(session, message);
      break;
    case "AgentTyping":
    ...
    case "ChasitorSessionData":
      ...
      break;
    case "ChatEnded":
      onChatEnded(session, message);
      break;
    case "ChatEstablished":
      onChatEstablished(session, message);
      break;
    case "ChatRequestFail":
      onChatRequestFail(session);
      break;
    case "ChatRequestSuccess":
      onChatRequestSuccess(session, message);
      break;
    ...
    case "TransferToButtonInitiated":
      onTransferToButtonInitiated(session);
      break;
    default:
      break;
  }
...
const onRichMessage = async (session, message) => {
  info("LIVEAGENT:onChatMessage:オペレーターからメッセージを受信");
  let actions = [];
  message.message.items.forEach((i) =>
    actions.push({
      type: "postback",
      label: i.text,
      data: i.text,
...
const onAgentDisconnect = async (session) => {
  ...
  await Router.processEvent(
    // createEvent(session, "message", { type: "text", text: "[自動送信] オペレータとの接続が切断しました。" })
...
const onChatEnded = async (session, message) => {
  if (message.message.reason == "chatbotEndedChat") {
    ...
    await Router.processEvent(
      // createEvent(session, "message", { type: "text", text: "[自動送信] オペレータがチャットを終了しました。" })
...
const onChatEstablished = async (session, message) => {
  if (message.message.userId.startsWith("0Xx")) {
    ...
    await Router.processEvent(
      // createEvent(session, "message", { type: "text", text: "[自動送信] オペレータと接続されました。" })
    ...
    await Router.processEvent(
      // createEvent(session, "message", { type: "text", text: "[自動送信] オペレータと接続されました。" })
      createEvent(session, "message", { type: "text", text: "オペレータと接続されました。" })
    );
  }
};
const onChatRequestFail = async (session) => {
  ...
  await Router.processEvent(
    // createEvent(session, "message", { type: "text", text: "[自動送信] 現在対応可能なオペレータがいません。" })
    createEvent(session, "message", { type: "text", text: "現在対応可能なオペレータがいません。" })
  );
  endMonitorChatActivity(session);
};
const onChatRequestSuccess = async (session, message) => {
  if (message.message.queuePosition > 0) {
    info("LIVEAGENT:onChatRequestSuccess:オペレータを呼び出しています");
    await Router.processEvent(
      //   createEvent(session, "message", { type: "text", text: "[自動送信] オペレータを呼び出しています。" })
